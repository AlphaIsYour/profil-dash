                ...
                <table class="table table-bordered">
                  ...
                  <tbody>
                      <?php
                        ...
                        $count_query = "SELECT COUNT(*) as total FROM `master_jenjang`";
                        if(!empty($search_query)) {
                            $count_query .= " WHERE `jenjang` LIKE ?";
                        }
                        ...
                        if(mysqli_num_rows($result) > 0) {
                            while($data_u = mysqli_fetch_assoc($result)){
                                $id_master_jenjang = $data_u['id_master_jenjang'];
                                $jenjang = $data_u['jenjang'];
                        ?>
                    <tr>
                    <td><?php echo $no;?></td>
                    <td><?php echo htmlspecialchars($jenjang);?></td>
                    <td align="center">
                    <a href="editjenjang.php?data=<?php echo htmlspecialchars($id_master_jenjang);?>"
                        class="btn btn-xs btn-info"><i class="fas fa-edit"></i> Edit</a>
                        <a href="javascript:if(confirm('Anda yakin ingin menghapus data <?php echo htmlspecialchars($jenjang); ?>?'))window.location.href = 'jenjang.php?aksi=hapus&data=<?php echo htmlspecialchars($id_master_jenjang);?>'"
